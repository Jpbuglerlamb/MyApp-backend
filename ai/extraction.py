...
# Canonical display roles (what you show/store)
ROLE_SYNONYMS: Dict[str, str] = {
    ...
    # Hospitality (IMPORTANT: keep waiter as Waiter)
    "waiter": "Waiter",
    "waitress": "Waiter",
...
def _strip_fillers(text: str) -> str:
    """Remove common filler words so AI extraction gets cleaner input."""
    t = (text or "").lower()
    fillers = [
        "i'm",
        "im",
...
def map_role_synonym(role_text: str, cutoff: float = 0.72) -> str:
    """Convert cleaned role text into a canonical display role."""
    if not role_text:
        return ""
...
def normalize_role_for_api(role: str) -> str:
    """
    Synchronous normalizer for API search queries (Adzuna).
    Returns a safe keyword string (space-separated).
    """
    role = (role or "").strip()
    if not role:
        return ""

    # Canonicalize to job-only form
    role = canonicalize_role(role)

    # Optional dataset resolution
    role = resolve_role_from_dataset(role) or role

    # Build safe search keywords
    return build_search_keywords(role).strip()


async def normalize_role_with_api(role: str) -> str:
    """
    AI cleanup that returns ONLY a job title (no schedule/contract modifiers).
    Output is canonicalized via canonicalize_role().
    """
    role = (role or "").strip()
    ...
    try:
        ...
        # Fail safe: never crash normalization
        return canonicalize_role(role)
...
async def extract_signals(message: str, state: Dict[str, Any]) -> None:
    ...
    # 2) Extract role & location using AI
